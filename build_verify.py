...
function renderSidebar() {{
  ...
  for (const level of LEVELS) {{
    ...
    for (const c of group) {{
      ...
      html += `<div class="sidebar-item${{active}}" data-cid="${{safeId}}" onclick="selectCandidate(this.dataset.cid)">
    ...
    for (const lt of LINK_TYPES) {{
      ...
      <div class="link-row${{rowCls}}" id="row-${{lt.key}}">
        ...
        <div class="link-body">
          ...
          ${{ls.status === 'bad' ? `<input class="edit-field" id="edit-${{lt.key}}" placeholder="Enter correct URL..." value="${{ls.correctedUrl || ''}}" onchange="updateCorrection(currentId,'${{lt.key}}',this.value)">` : ''}}
        </div>
        <div class="link-actions">
          <button class="btn btn-open" data-url="${{url.replace(/"/g,'&quot;')}}" onclick="window.open(this.dataset.url,'_blank')">Open ↗</button>
          <button class="btn btn-ok${{okActive}}" data-url="${{url.replace(/"/g,'&quot;')}}" onclick="markLink(currentId,'${{lt.key}}','ok',this.dataset.url)">✓</button>
          <button class="btn btn-bad${{badActive}}" data-url="${{url.replace(/"/g,'&quot;')}}" onclick="markLink(currentId,'${{lt.key}}','bad',this.dataset.url)">✗</button>
        </div>
      </div>`;
    }}
  }}
  ...
  <div class="candidate-card">
    ...
    <div class="add-link-section">
      ...
      <div class="add-link-row">
        ...
        <button class="btn-add" onclick="submitAddLink()">Add</button>
      </div>
    </div>` : ''}}
    <div class="nav-row">
